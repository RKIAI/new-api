...
func UpdateSunoTaskAll(ctx context.Context, taskChannelM map[int][]string, taskM map[string]*model.Task) error {
	for channelId, taskIds := range taskChannelM {
		...
		if err != nil {
			common.LogError(ctx, fmt.Sprintf("Channel #%d update asynchronous task failed: %d", channelId, err.Error()))
		}
	}
	return nil
}
...
func updateSunoTaskAll(ctx context.Context, channelId int, taskIds []string, taskM map[string]*model.Task) error {
	common.LogInfo(ctx, fmt.Sprintf("Channel #%d has unfinished tasks: %d", channelId, len(taskIds)))
	if len(taskIds) == 0 {
		return nil
	}
	...
	if !responseItems.IsSuccess() {
		common.SysLog(fmt.Sprintf("Channel #%d has unfinished tasks: %d, successfully obtained tasks: %d", channelId, len(taskIds), string(responseBody)))
		return err
	}
	...
	for _, responseItem := range responseItems.Data {
		...
		if responseItem.FailReason != "" || task.Status == model.TaskStatusFailure {
			common.LogInfo(ctx, task.TaskID+" Build failure,"+task.FailReason)
			task.Progress = "100%"
			...
			if err != nil {
				...
				if quota != 0 {
					...
					logContent := fmt.Sprintf("Asynchronous task execution failed %s, compensation %s", task.TaskID, common.LogQuota(quota))
