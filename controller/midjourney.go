...
func UpdateMidjourneyTaskBulk() {
	...
	for {
		...
		common.LogInfo(ctx, fmt.Sprintf("The number of unfinished tasks detected is: %v", len(tasks)))
		...
		for channelId, taskIds := range taskChannelM {
			common.LogInfo(ctx, fmt.Sprintf("The unfinished tasks for channel #%d are: %d", channelId, len(taskIds)))
			if len(taskIds) == 0 {
				continue
			}
			...
			for _, responseItem := range responseItems {
				...
				if (task.Progress != "100%" && responseItem.FailReason != "") || (task.Progress == "100%" && task.Status == "FAILURE") {
					common.LogInfo(ctx, task.MjId+" Build failure，"+task.FailReason)
					task.Progress = "100%"
					if task.Quota != 0 {
				...
				if err != nil {
					...
					if shouldReturnQuota {
						...
						logContent := fmt.Sprintf("Composition failed %s, compensation %s", task.MjId, common.LogQuota(task.Quota))
