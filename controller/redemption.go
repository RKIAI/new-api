...
func AddRedemption(c *gin.Context) {
	...
	if len(redemption.Name) == 0 || len(redemption.Name) > 20 {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "The length of the redemption code name must be between 1 and 20 characters.",
		})
		return
	}
	...
	if redemption.Count > 100 {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": "The number of redemption codes generated in batches cannot exceed 100",
		})
		return
	}
	var keys []string
